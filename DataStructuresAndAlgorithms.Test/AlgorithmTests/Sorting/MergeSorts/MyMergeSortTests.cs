...
namespace DataStructuresAndAlgorithms.Test.AlgorithmTests.Sorting.MergeSorts
{
    public class MyMergeSortTests
    {
        ...
        public void MyMergeSortedIntArray_Equals_ExpectedSortedIntArray()
        {
            ...
            int[] actual = arr.MergeSort();

            Assert.Equal(expected, actual);
        }

        [Fact]
        public void MyMergeSortedStringArray_Equals_ExpectedSortedStringArray()
        {
            string[] arr = { "sit", "down", "on", "the", "ground", "david" };
            string[] expected = { "david", "down", "ground", "on", "sit", "the" };

            string[] actual = arr.MergeSort();

            Assert.Equal(expected, actual);
        }

        [Fact]
        public void MyMergeSortedFloatArray_Equals_ExpectedSortedFloatArray()
        {
            ...
            float[] actual = arr.MergeSort();

            Assert.Equal(expected, actual);
        }

        [Fact]
        public void MyMergeSortedCharArray_Equals_ExpectedSortedCharArray()
        {
            char[] arr = { 'd', 'z', 'w', '3', 'f', 'z', 'g', 'b', 't' };
            char[] expected = { '3', 'b', 'd', 'f', 'g', 't', 'w', 'z', 'z' };

            char[] actual = arr.MergeSort();

            Assert.Equal(expected, actual);
        }

        [Fact]
        public void MyMergeSortTestClassOneArray_Equals_ExpectedSortedByIdArray()
        {
            TestClassOne[] arr = TestClassOneService.GenerateTestClassOnesArray();
            TestClassOne[] expected = arr.OrderBy(t => t.Id).ToArray();

            TestClassOne[] actual = arr.MergeSortBy(t => t.Id);

            Assert.Equal(expected, actual);
        }

        [Fact]
        public void MyMergeSortTestClassOneArray_Equals_ExpectedSortedByNameArray()
        {
            TestClassOne[] arr = TestClassOneService.GenerateTestClassOnesArray();
            TestClassOne[] expected = arr.OrderBy(t => t.Name).ToArray();

            TestClassOne[] actual = arr.MergeSortBy(t => t.Name);
